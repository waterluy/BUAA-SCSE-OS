...
diskaddr(u_int blockno)
{
    if (super && (blockno >= super->s_nblocks)) {
        user_panic("diskaddr non-existent block %08x\n", blockno);
    } else {
        return (DISKMAP + 4096 * blockno);
    }
}
...
int file_listdir(const char *path, char *ans) {
    char name[MAXNAMELEN];
    int r;
    struct File *ddir, *dir, f;
    u_int i, j, nblock;
    void *blk;

    if ((r = walk_path(path,  &ddir, &dir, name)) < 0) {
        return r;
    }
    ...
    for (i = 0; i < nblock; i++) {
        ...
        f = (struct File *)blk;
        for (j = 0; j < FILE2BLK; ++j) {
            if (f[j].f_type == FTYPE_REG) {
                strcpy(ans, f[j].f_name);
                ans += strlen(f[j].f_name);
                *ans = ' ';
                ans++;
            }
        }
