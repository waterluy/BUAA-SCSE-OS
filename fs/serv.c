...
void serve_listdir(u_int envid, struct Fsreq_listdir *rq) {
    int r;
    u_char path[MAXPATHLEN];
    char temp[MAXPATHLEN];
    // Step 1: Copy in the path, making sure it's terminated.
    // Notice: add \0 to the tail of the path
    user_bcopy(path, rq->req_path, MAXPATHLEN);
    path[MAXPATHLEN - 1] = '\0';
    if ((r = file_listdir(path, temp)) < 0) {
        ipc_send(envid, r, 0, 0);
    }
    ipc_send(envid, 0, (u_int)temp, PTE_V | PTE_R | PTE_LIBRARY);
}
