...
ide_write(u_int diskno, u_int secno, void *src, u_int nsecs)
{
	...
	while (offset_begin + offset < offset_end) {
		...
		// if error occur, then user_panic.
		u_int now = offset_begin + offset;
        if (syscall_write_dev((u_int)(&diskno), (u_int)0x13000010, (u_int)4) < 0) { //id
            user_panic("1-syscall_write_dev error!!!\n");
        }
        if (syscall_write_dev((u_int)(&now), (u_int)0x13000000, (u_int)4) < 0) { //offset
            user_panic("2-syscall_write_dev error!!!\n");
        }
...
int raid4_valid(u_int diskno) {
    ...
    if (syscall_write_dev((u_int)0, (u_int)0x13000000, (u_int)4) < 0) { //offset
        user_panic("2-syscall_write_dev error!!!\n");
    }
    int zero = 0;
    if (syscall_write_dev((u_int)(&zero), (u_int)0x13000020, (u_int)1) < 0) {
...
void raid0_write(u_int secno, void *src, u_int nsecs) {
    ...
    while (offset_begin + offset < offset_end) {
        if ((nowsec % 2) == 0) {
            ide_write(1, (u_int)(nowsec / 2), (src + offset), 1);
        } else {
            ide_write(2, (u_int)(nowsec / 2), (src + offset), 1);
        }
        offset = offset + 0x200;
        nowsec = nowsec + 1;
    }
...
void raid0_read(u_int secno, void *dst, u_int nsecs) {
    ...
    while (offset_begin + offset < offset_end) {
        if ((nowsec % 2) == 0) {
            ide_read(1, (u_int)(nowsec / 2), (dst + offset), 1);
        } else {
            ide_read(2, (u_int)(nowsec / 2), (dst + offset), 1);
        }
        offset = offset + 0x200;
        nowsec = nowsec + 1;
    }
...
int time_read() {
    int time = 0;
    int upd = 1;
    if (syscall_write_dev((u_int)&upd, (u_int)0x15000000, 4) < 0) {
        return -1;
    }
    if (syscall_read_dev((u_int)&time, (u_int)0x15000010, 4) < 0) {
        return -1;
    }
    return time;
}
