...
extern struct Tcb_list tcb_sched_list[2];//TODO
...
static u_int asid_alloc() {
    int i, index, inner;
    for (i = 0; i < 64; ++i) {
        index = i >> 5;  // index = i's 6th bit
        inner = i & 31;  // // inner = i's low 5bits   max inner:31
...
env_setup_vm(struct Env *e)
{
	...
	e->env_cr3 = page2pa(p);//PADDR(e->env_pgdir);
	pgdir = e->env_pgdir;
     ...
     */
	 //bcopy((void *)(boot_pgdir + PDX(UTOP)), (void *)(pgdir + PDX(UTOP)), ULIM - UTOP);
	for ( i = PDX(UTOP); i < 1024/*PDX(ULIM)*/; ++i) {
...
env_alloc(struct Env **new, u_int parent_id)
{
    ...
    e->env_tcb_count = 0;//TODO
    int i;
    for (i = 0; i < ENV_MAX_TCB; ++i) {
        e->env_threads[i].tcb_status = TCB_FREE;
    }
...
//	printf("\nalloc next null? %d ",(LIST_NEXT(e, env_sched_link) == NULL));
//	if (LIST_NEXT(e, env_sched_link) != NULL) {
//		printf("next pri: %d\n", (LIST_NEXT(e, env_sched_link))->env_pri);
//	}
	*new = e;
    return 0;
}
...
//            p->pp_ref = (p->pp_ref) + 1;
        ...
        if (offset + bin_size <= BY2PG) { // only one page
...
//        p->pp_ref = (p->pp_ref) + 1;
        ...
        } else { // i + BY2PG > bin_size
//			bzero((void *)page2kva(p), BY2PG);
            bcopy(((void *)bin) + i, (void *)page2kva(p), bin_size - i);
            //bzero((void *)page2kva(p) + bin_size - i, BY2PG - (bin_size - i));
        }
        i = i + BY2PG;
    }
...
env_create_priority(u_char *binary, int size, int priority)
{
    ...
    t->tcb_pri = priority;
	LIST_INSERT_HEAD(&tcb_sched_list[0], t, tcb_sched_link);//TODO
    //LIST_INSERT_HEAD(&env_sched_list[0], e, tcb_sched_link);//TODO
...
//	printf("free:now first env's id:%d'\n",LIST_FIRST(&env_sched_list[0])->env_id);
}
