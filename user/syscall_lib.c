...
int syscall_get_thread_id(struct TCB **t) {
    return msyscall(SYS_get_thread_id, t, 0, 0, 0, 0);
}
...
int syscall_set_thread_status(u_int tcbid, u_int status) {
...
int syscall_thread_join(u_int tcbid, void **retval) {
...
int syscall_sem_try_wait(sem_t *sem) {
    return msyscall(SYS_sem_try_wait, sem, 0, 0, 0, 0);
}

int syscall_sem_get_value(sem_t *sem, int *sval) {
    return msyscall(SYS_sem_get_value, sem, sval, 0, 0, 0);
}

void syscall_putchar(char ch)
{
	msyscall(SYS_putchar, (int)ch, 0, 0, 0, 0);//include/unistd.h....user/syscall_wrap.S
}
