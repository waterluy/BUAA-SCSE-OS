...
usr_load_elf(int fd , Elf32_Phdr *ph, int child_envid){
    ...
    u_long binsize = ph->p_filesz;
    int r;
    u_char *blk;
    u_char *tmp = USTACKTOP;

    if ((r = read_map(fd, ph->p_offset, &blk)) < 0) {
        return r;
    }
    int i;
    ...
    int size = 0;
    if (offset) {
        if ((r = syscall_mem_alloc(child_envid, va, PTE_R | PTE_V)) < 0) {
            return r;
        }
        if ((r = syscall_mem_map(child_envid, va, 0, tmp, PTE_R | PTE_V)) < 0) {
            return r;
        }
        size = MIN(BY2PG - offset, binsize);
        user_bcopy(blk, tmp + offset, size);
        if ((r = syscall_mem_unmap(0, tmp)) < 0) {
            return r;
        }
    }
    for (i = size; i < binsize; i += BY2PG) {
        if ((r = syscall_mem_alloc(child_envid, va + i, PTE_R | PTE_V)) < 0) {
            return r;
        }
        if ((r = syscall_mem_map(child_envid, va + i, 0, tmp, PTE_R | PTE_V)) < 0) {
            return r;
        }
        user_bcopy(blk + i, tmp, MIN(BY2PG, binsize - i));
        if ((r = syscall_mem_unmap(0, tmp)) < 0) {
            return r;
        }
    }
...
int spawn(char *prog, char **argv)
{
	...
	if ((r = init_stack(child_envid, argv, &esp)) < 0) {
	    user_panic("spawn ::init_stack wrong!\n");
	}
	...
	for (i = 0; i < elf->e_phnum; ++i) {
        if (ph->p_type == PT_LOAD) {
            if ((r = user_load_elf(fd, ph, child_envid)) < 0) {
                return r;
            }
        }
        ph++;
