...
struct Page *pages; // include/pmap.h
...
void mips_detect_memory()
{
	...
	basemem = 64 * 1024 * 1024; // 67108864 Bytes 0x04000000
	// Step 2: Calculate corresponding npage value.
	maxpa = basemem;
	npage = basemem / 4096; // page: 4KB = 4096B
	extmem = 0;
...
{
	extern char end[]; // v [0x80400000, 0x83ffffff] p[0x00400000, 0x03ffffff]
	u_long alloced_mem;
	...
	if (freemem == 0) {
		freemem = (u_long)end;
	} // [0x80400000, freemem - 1]的虚拟地址已经被分配好了

	/* Step 1: Round up `freemem` up to be aligned properly */
	freemem = ROUND(freemem, align); // in include/types.h

	/* Step 2: Save current value of `freemem` as allocated chunk. */
	alloced_mem = freemem; // 初始虚拟地址

	/* Step 3: Increase `freemem` to record allocation. */
	freemem = freemem + n;

	/* Check if we're out of memory. If we are, PANIC !! */
	if (PADDR(freemem) >= maxpa) { // in include/mmu.h
		panic("out of memory\n");
	...
	if (clear) {
		bzero((void *)alloced_mem, n);  // in init/init.c
	}
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	for (i = 0; i < size; i += BY2PG) {
		...
		*pgtable_entry = (pa + i) | perm | PTE_V; // PTE_ADDR(pa + i)???
	}
}
...
{
	extern char end[];
	extern int mCONTEXT;  // boot/start.S   mCONTEXT: .word 0
	extern struct Env *envs;

	Pde *pgdir; // Pde u_long
	u_int n;

	/* Step 1: Allocate a page for page directory(first level page table). */
	pgdir = alloc(BY2PG, BY2PG, 1);  //BY2PG 4096  pgdir为初始虚拟地址
	...
	// pages 是 npage 个 Page 的起始地址
	pages = (struct Page *)alloc(npage * sizeof(struct Page), BY2PG, 1);  // include/pmap.h
...
void page_init(void)
{
	 ...
	 * filed to 1) */
	int pa = 0;
	for (pa = 0; pa < PADDR(freemem); pa = pa + BY2PG) {
		(pa2page(pa))->pp_ref = 1;
	}
	/*
	struct Page *initpage;
	for (initpage = pages; page2kva(initpage) < freemem; ++initpage) {
		initpage->pp_ref = 1;
	}*/

	/* Step 4: Mark the other memory as free. */
	for (pa = PADDR(freemem); pa < maxpa; pa = pa + BY2PG) {
		(pa2page(pa))->pp_ref = 0;
		LIST_INSERT_HEAD((&page_free_list), (pa2page(pa)), pp_link);
	}
	/*
	for (initpage = &pages[PPN(PADDR(freemem))]; page2ppn(initpage) < npage; ++initpage) {
		initpage->pp_ref = 0;
		LIST_INSERT_HEAD((&page_free_list), initpage, pp_link);
	}*/
}
...
int page_alloc(struct Page **pp) // 传地址才能改变值
{
...
void pageout(int va, int context)
{
	u_long r;
	struct Page *p = NULL;
	// tlb 缺失时，mCONTEXT 不位于 kseg0 则说明系统出现了故障
	if (context < 0x80000000) {
		panic("tlb refill and alloc error!");
	}
	// 引发 tlb 缺失的虚拟地址不合法(访问了过低或过高的地址)，则说明系统鼓掌
