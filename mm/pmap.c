...
Pde *boot_pgdir; // Pde u_long  (mmu.h)
...
void mips_detect_memory()
{
	...
	basemem = 64 * 1024 * 1024; // 67108864 Bytes 0x4000000
...
void mips_vm_init()
{
	extern char end[]; //tools/scse0_3.lds
	extern int mCONTEXT;
	...
	pgdir = alloc(BY2PG, BY2PG, 1); //BY2PG 4096
	...
	n = ROUND(NENV * sizeof(struct Env), BY2PG); // NENV 1<<10
...
int page_alloc(struct Page **pp)
{
	...
	if (LIST_EMPTY((&page_free_list))) {
		return -E_NO_MEM; // include/error.h
	}
	...
	bzero((void *)page2kva(ppage_temp), BY2PG); // include/pmap.c   init/init.c
	*pp = ppage_temp;
	return 0;
...
void pageout(int va, int context)
{
	...
	if (va < 0x10000) {
		printf("pageout (TOO LOW)va:0x%x\n",va);
		panic("^^^^^^TOO LOW^^^^^^^^^");
	}
