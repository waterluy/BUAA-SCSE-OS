...
LIST_HEAD(Page_list, Page);  // include/queue.h
typedef LIST_ENTRY(Page) Page_LIST_entry_t; // include/queue.h
...
struct Page {
	...
	u_short pp_ref; // 引用次数 等价于有多少虚页映射的该物理页
};
...
{
	return page2ppn(pp) << PGSHIFT;  // include/mmu.h
}
...
pa2page(u_long pa)
{
	...
	return &pages[PPN(pa)];  // include/mmu.h
}
